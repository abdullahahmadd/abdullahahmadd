# 👋 Hi, I'm Abdullah Ahmad
### Data Analyst | Business Intelligence | Database Administration

![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?logo=powerbi&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?logo=mysql&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?logo=postgresql&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?logo=microsoft-excel&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?logo=r&logoColor=white)

---

## 📊 About Me

Data Analyst with 3+ years of experience across data analytics, business intelligence, and database administration. I work across the full data lifecycle — collection, cleaning, validation, analysis, visualization, and reporting — turning raw operational data into dashboards and insights that support real business decisions.

My recent work includes building a full Python analytics pipeline on 1,200+ retail transactions (uncovering SAR 4.37M in sales), designing star-schema Power BI dashboards with custom DAX measures, and architecting a 470,000+ record PostgreSQL database for a simulated construction company.

I enjoy solving business problems with data and building practical, well-documented solutions.

---

## 🚀 What I Build

* Interactive Power BI dashboards and KPI reporting solutions
* Python/R analytics pipelines — data cleaning, EDA, feature engineering
* SQL-based analytics and normalized database systems
* Excel reporting and automation solutions
* Data warehouse and ETL workflows

---

## 🛠️ Technical Skills

**Data Analytics & BI**
Power BI · DAX · Power Query · MS Excel · Tableau · Google Looker Studio · EDA

**Databases**
PostgreSQL · MySQL · SQL Query Optimization · Data Modeling · Data Warehousing · ERD & Normalization

**Programming**
Python (Pandas, NumPy, Matplotlib, Seaborn) · SQL · R · Excel VBA

---

## 🎓 Certifications

* Google Data Analytics Specialization
* Google Advanced Data Analytics Specialization
* Google Business Intelligence Specialization
* IBM Data Engineering Specialization
* Microsoft Power BI Data Analyst Specialization
* Microsoft Generative AI for Data Analysis Specialization
* Lean Six Sigma (White, Yellow, Green & Black Belt)
* AWS Cloud Practitioner
* Microsoft Azure Fundamentals

📄 Full list of certifications and additional projects on my [LinkedIn](https://www.linkedin.com/in/aabdullah-ahmad) and [GitHub repositories](https://github.com/abdullahahmadd?tab=repositories).

---

## 🤝 Connect With Me

📍 Jeddah, Saudi Arabia
💼 Open to **Data Analyst**, **BI Analyst/Developer**, and **Database Administrator** opportunities.

📧 abdullah.ahmadk0@gmail.com | 🔗 [LinkedIn](https://www.linkedin.com/in/aabdullah-ahmad)

Thanks for visiting my profile!
